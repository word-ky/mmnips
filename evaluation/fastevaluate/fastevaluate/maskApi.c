...
void rleEncode( RLE *R, const bytee *M, siz h, siz w, siz n ) {
  siz i, j, k, a=w*h; uint c, *cnts; bytee p;
  cnts = (uint*)malloc(sizeof(uint)*(a+1));
  for(i=0; i<n; i++) {
    const bytee *T=M+a*i; k=0; p=0; c=0;
...
void rleDecode( const RLE *R, bytee *M, siz n ) {
  siz i, j, k; for( i=0; i<n; i++ ) {
    bytee v=0; for( j=0; j<R[i].m; j++ ) {
...
void rleIou( RLE *dt, RLE *gt, siz m, siz n, bytee *iscrowd, double *o ) {
...
void bbIou( BB dt, BB gt, siz m, siz n, bytee *iscrowd, double *o ) {
